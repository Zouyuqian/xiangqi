...
public class ChessBoardPanel extends JPanel {
    ...
    public boolean saveGame() {
        if (userManager.getCurrentUser().isGuest()) {
            JOptionPane.showMessageDialog(this, "游客无法保存游戏！请注册账号后使用此功能。", "错误", JOptionPane.ERROR_MESSAGE);
            return false;
        }

        try {
            GameSave save = model.createSave();
            userManager.getCurrentUser().setCurrentSave(save);
    ...
    public boolean loadGame() {
        ...
        GameSave save = userManager.getCurrentUser().getCurrentSave();
        if (save == null) {
            JOptionPane.showMessageDialog(this, "没有找到保存的游戏！", "错误", JOptionPane.INFORMATION_MESSAGE);
            return false;
        }
    ...
    private void drawGameStatus(Graphics g) {
        ...
        g.drawString(userInfo, MARGIN + 400, 20);
    }
}